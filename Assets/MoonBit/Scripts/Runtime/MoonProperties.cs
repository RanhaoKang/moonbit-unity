...
namespace UnityEditor.GUI
{
    ...
    [CustomEditor(typeof(MoonProperties))]
    //[CanEditMultipleObjects]
    public class MoonProperties_CustomEditor : Editor
    {
        private Object _scriptObject;
        //实例引用
        private MoonProperties _instance;
        ...
        void OnEnable()
        {
            //Debug.Log("OnEnable");
            _scriptObject = null;
            _instance = target as MoonProperties;
            if (_instance == null)
                return;

            //script object
            var targetName = $"/{nameof(MoonProperties)}.cs";
            ...
            display.AddDrawer<StringDrawer>(typeof(MoonProperties.String));
            display.AddDrawer<NumberDrawer>(typeof(MoonProperties.Number));
            display.AddDrawer<BooleanDrawer>(typeof(MoonProperties.Boolean));
            display.AddDrawer<ObjectDrawer>(typeof(MoonProperties.Object));
            display.AddDrawer<ObjectArrayDrawer>(typeof(MoonProperties.ObjectArray));
            display.AddDrawer<GameObjectArrayDrawer>(typeof(MoonProperties.GameObjectArray));
        ...
        private DrawerInfo CreateDrawerInfo()
        {
            ...
            if (_instance != null)
            {
                ...
                var fields = typeof(MoonProperties).GetFields(BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Public);
                foreach (var field in fields)
                {
                    Type elementType = field.FieldType.IsArray ? field.FieldType.GetElementType() : null;
                    if (elementType != null
                        && typeof(MoonProperties.IPair).IsAssignableFrom(elementType)
                        && serializedObject.FindProperty(field.Name) != null
                        && DisplayUtility.CheckFields(elementType))
                    {

                        var options = elementType.GetCustomAttribute<MoonProperties.OptionsAttribute>(false);
        ...
        void DisplayMenu()
        {
            ...
            if (menuFoldout)
            {
                ...
                {
                    MoonProperties.ResultPair[] pairs = _instance.GenPairs();
                    if (pairs != null)
                    {
    ...
    public class DisplayUtility
    {
        ...
        {
            FieldInfo index, key;
            return typeof(MoonProperties.IPair).IsAssignableFrom(type)
        ...
        public static string GenCode(MoonProperties.ResultPair[] pairs, string declareType, bool useFullname)
        {
            var resultCode = "";
        ...
        public static void PraseCode(MoonProperties_CustomEditor editor, string code)
        {
